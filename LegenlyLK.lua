--[[
    Версия: 5.0
    Название: Legenly LK
    Главная фича: FE Sounds (глобальные звуки для всех в Brookhaven)
    Дополнительно: Bring, Kill, Fling, ESP, Speed, NoClip, Fly
]]
...
makeLabel(header, UDim2.new(0.25,0,1,0), UDim2.new(0.7,0,0,0), "v5.0", colors.textdim, 12)
...
-- ========== ПОИСК REMOTEEVENT ДЛЯ ЗВУКОВ ==========
local function findSoundRemote()
    for _, child in pairs(ReplicatedStorage:GetDescendants()) do
        if child:IsA("RemoteEvent") then
            local name = child.Name
            if name:find("Sound") or name:find("Audio") or name:find("Message") or name:find("Gun") or name:match("^%d") then
                return child
            end
        end
    end
...
-- Функция воспроизведения звука для всех
local function playGlobalSound(soundId)
    ...
    local success, err = pcall(function()
        soundRemote:FireServer("PlaySound", soundId)
        wait(0.1)
        soundRemote:FireServer("PlayAudio", soundId)
        wait(0.1)
        soundRemote:FireServer("Sound", soundId)
        wait(0.1)
        soundRemote:FireServer("GlobalSound", soundId)
        wait(0.1)
...
-- Создаём кнопки с проверенными ID (скример, сирена, стон и т.д.)
...
local playCustomBtn = makeBtn(contentContainer, UDim2.new(0.2,0,0.06,0), UDim2.new(0.56,0,0.57,0), "▶ Воспроизвести", colors.accent, colors.text, 6)
...
-- ========== ОСТАЛЬНЫЕ ФУНКЦИИ (BROOKHAVEN) ==========

-- Блок быстрых действий (Bring, Kill, Fling)
local actionLabel = makeLabel(contentContainer, UDim2.new(0.94,0,0.06,0), UDim2.new(0.03,0,0.66,0), "⚡ БЫСТРЫЕ ДЕЙСТВИЯ", colors.gold, 13)
local actionGrid = makeFrame(contentContainer, UDim2.new(0.94,0,0.15,0), UDim2.new(0.03,0,0.74,0), colors.panel, 0, 6)

local function createActionBtn(parent, name, col, color)
    local btn = makeBtn(parent, UDim2.new(0.30,0,0.8,0),
        UDim2.new(0.02 + (col * 0.33), 0, 0.1, 0),
        name, color or colors.accent, colors.text, 6)
    btn.TextSize = 11
    return btn
end

local bringAllBtn = createActionBtn(actionGrid, "📦 Bring All", 0, colors.green)
local killAllBtn = createActionBtn(actionGrid, "💀 Kill All", 1, colors.red)
local flingAllBtn = createActionBtn(actionGrid, "🌀 Fling All", 2, colors.gold)

bringAllBtn.MouseButton1Click:Connect(function()
    local root = player.Character and player.Character:FindFirstChild("HumanoidRootPart")
    if not root then statusText.Text = "❌ Нет персонажа"; return end
...
-- Тогглы (ESP, Speed, NoClip, Fly)
local togglesLabel = makeLabel(contentContainer, UDim2.new(0.94,0,0.06,0), UDim2.new(0.03,0,0.9,0), "🛠 ДОПОЛНИТЕЛЬНО", colors.gold, 13)
local togglesGrid = makeFrame(contentContainer, UDim2.new(0.94,0,0.15,0), UDim2.new(0.03,0,0.96,0), colors.panel, 0, 6)
...
print("⚡ Legenly LK v5.0 загружен (FE Sounds Edition)")
