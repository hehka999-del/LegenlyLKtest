...
local musicSound = nil  -- для глобальной музыки
...
-- Цветовая схема (тёмная, без яркости)
local colors = {
...
local function makeFrame(parent, size, pos, color, trans, radius)
    ...
    f.Size = size; f.Position = pos; f.BackgroundColor3 = color or colors.bg
    f.BackgroundTransparency = trans or 0; f.BorderSizePixel = 0
    f.Parent = parent
    if radius then
...
local function makeBtn(parent, size, pos, text, color, txtColor, radius)
    ...
    b.Size = size; b.Position = pos; b.BackgroundColor3 = color or colors.panel
    b.BackgroundTransparency = 0; b.BorderSizePixel = 0
    b.Text = text or ""; b.TextColor3 = txtColor or colors.text
    b.TextScaled = false; b.TextSize = 14; b.Font = Enum.Font.GothamBold
    b.Parent = parent
    if radius then
...
local function makeLabel(parent, size, pos, text, color, sizeText)
    ...
    l.Text = text or ""; l.TextColor3 = color or colors.text
    l.TextScaled = false; l.TextSize = sizeText or 14; l.Font = Enum.Font.Gotham
    l.Parent = parent
    return l
end
...
local function makeTextBox(parent, size, pos, placeholder, color)
    ...
    t.Size = size; t.Position = pos; t.BackgroundColor3 = color or colors.panel
    t.BackgroundTransparency = 0; t.BorderSizePixel = 0
    t.Text = placeholder or ""; t.TextColor3 = colors.textdim
    t.TextScaled = false; t.TextSize = 14; t.Font = Enum.Font.Gotham
...
-- ================== ОКНО ВХОДА ==================
local loginFrame = makeFrame(gui, UDim2.new(0, 340, 0, 280), UDim2.new(0.5, -170, 0.5, -140), colors.bg, 0, 12)

-- Заголовок
local header = makeFrame(loginFrame, UDim2.new(1, 0, 0, 45), UDim2.new(0,0,0,0), colors.bg2, 0, 12)
makeLabel(header, UDim2.new(0.6,0,1,0), UDim2.new(0.04,0,0,0), "⚡ Legenly LK", colors.gold, 18)
makeLabel(header, UDim2.new(0.2,0,1,0), UDim2.new(0.7,0,0,0), "v2.0", colors.textdim, 14)

-- Кнопка закрытия (всегда активна)
local closeBtn = makeBtn(header, UDim2.new(0, 28, 0, 28), UDim2.new(1, -34, 0, 8), "✕", colors.bg2, Color3.fromRGB(255,120,120), 6)
closeBtn.MouseButton1Click:Connect(function() gui:Destroy() end)

-- Подзаголовок
makeLabel(loginFrame, UDim2.new(0.9,0,0,24), UDim2.new(0.05,0,0,55), "Введи ключ", colors.textdim, 14)

-- Поле ввода
local keyBox = makeTextBox(loginFrame, UDim2.new(0.9,0,0,36), UDim2.new(0.05,0,0,85), "Вставь ключ...")

-- Кнопки
local copyBtn = makeBtn(loginFrame, UDim2.new(0.42,0,0,36), UDim2.new(0.05,0,0,135), "📋 Copy", colors.gold, Color3.fromRGB(0,0,0), 6)
local verifyBtn = makeBtn(loginFrame, UDim2.new(0.42,0,0,36), UDim2.new(0.53,0,0,135), "✅ Verify", colors.accent, colors.text, 6)

-- Статус
local statusLabel = makeLabel(loginFrame, UDim2.new(0.9,0,0,24), UDim2.new(0.05,0,0,185), "", colors.text, 14)

-- ================== ГЛАВНАЯ ПАНЕЛЬ ==================
local mainPanel = makeFrame(gui, UDim2.new(0, 420, 0, 480), UDim2.new(0.5, -210, 0.5, -240), colors.bg, 0, 12)
mainPanel.Visible = false

-- Заголовок панели
local panelHeader = makeFrame(mainPanel, UDim2.new(1,0,0,42), UDim2.new(0,0,0,0), colors.bg2, 0, 12)
makeLabel(panelHeader, UDim2.new(0.5,0,1,0), UDim2.new(0.04,0,0,0), "⚡ Legenly LK", colors.gold, 16)

-- Кнопка свернуть (минус) – сворачивает в полоску
local minBtn = makeBtn(panelHeader, UDim2.new(0, 28, 0, 28), UDim2.new(1, -68, 0, 7), "−", colors.bg2, colors.text, 6)
-- Кнопка закрыть (крестик)
local closePanelBtn = makeBtn(panelHeader, UDim2.new(0, 28, 0, 28), UDim2.new(1, -34, 0, 7), "✕", colors.bg2, Color3.fromRGB(255,120,120), 6)
...
-- Создание кнопок-тогглов (3 ряда по 2 или 3)
...
local musicIdBox = makeTextBox(musicFrame, UDim2.new(0.45,0,0.7,0), UDim2.new(0.28,0,0.15,0), "ID трека")
...
startMusicBtn.MouseButton1Click:Connect(function()
    ...
    -- Удаляем старый звук, если есть
    if musicSound and musicSound.Parent then musicSound:Destroy() end
    -- Создаём звук в Workspace с большим радиусом
